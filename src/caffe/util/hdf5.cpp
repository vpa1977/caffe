...
float hdf5_load_float<float>(hid_t loc_id, const string& dataset_name) {
	...
	CHECK_GE(status, 0) << "Failed to load int dataset with name " << dataset_name;
	return val;
}
...
void hdf5_save_float<float>(hid_t loc_id, const string& dataset_name, float f) {
	hsize_t one = 1;
	herr_t status = H5LTmake_dataset_float(loc_id, dataset_name.c_str(), 1, &one, &f);
	CHECK_GE(status, 0) << "Failed to save int dataset with name " << dataset_name;
}

template <>
void hdf5_save_float<double>(hid_t loc_id,const string& dataset_name, double f) {
	hsize_t one = 1;
	herr_t status = H5LTmake_dataset_double(loc_id, dataset_name.c_str(), 1, &one, &f);
	CHECK_GE(status, 0) << "Failed to save int dataset with name " << dataset_name;
}
